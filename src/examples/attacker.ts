 ...
 * @description  Juno Game Examples: Attacker
...
export class Game extends Juno.Game {
  ...
  init(): void {
    this.graphics.load("sprites", "./assets/examples/attacker/", 8);
    this.addBackground();
  }
  ...
  handleInput(): void {
    ...
    if (this.graphics.key(1)) {
      this.ship.y += 1;
    }
    if (this.graphics.key(2)) {
      this.ship.x -= 2;
    }
    if (this.graphics.key(3)) {
      this.ship.x += 2;
    }
    if (this.graphics.keyp(4)) {
      if (this.ship.s !== 0) {
        this.ship.s--;
        this.superBomb();
      }
    }
    if (this.graphics.keyp(5)) {
      this.fire();
    }
    if (this.graphics.keyp(6)) {
      if (this.ship.l !== 0) {
        this.ship.l--;
        this.laserAttack();
      }
    }
    if (this.graphics.keyp(7)) {
      if (this.ship.d !== 0) {
        this.ship.d--;
  ...
  drawObjects(): void {
    ...
    this.stars.forEach((s, i) => {
      s.y += s.s;
      this.graphics.pix(s.x, s.y, Math.floor(Math.random() * 10));
      if (s.y > this.graphics.ggh()) {
        s.y = 0;
      }
    });

    // draw our player
    this.graphics.anim(this.ship, 0, 2, 4);
    ...
    this.bullets.forEach((b, i) => {
      if (!this.bulletOfScreen(b)) {
        b.x += b.dx;
        b.y += b.dy;
        this.graphics.spr(b.sp, b.x, b.y);

        this.enemies.forEach((e, index) => {
          if (this.graphics.rrc(b, e)) {
            e.health -= 1;
    ...
    this.enemies.forEach((e, i) => {
      if (!this.enemyOfScreen(e)) {
        ...
        this.graphics.anim(e, e.sp, 2, 4);

        if (this.graphics.crc(this.superBombCircle, e)) {
          this.ship.p += 10;
          this.enemies.splice(i, 1);
          this.createCircleExplosion(e, 3);
        }

        if (this.graphics.rrc(e, this.laserLine)) {
          this.ship.p += 10;
          this.enemies.splice(i, 1);
          this.createCircleExplosion(e, 3);
        }

        if (this.graphics.rrc(e, this.ship)) {
          this.ship.p += 10;
          this.enemies.splice(i, 1);
          this.playerDying();
          this.createCircleExplosion(e, 3);
    ...
    this.explosionCircles.forEach((c, i) => {
      if (c.r < 30) {
        this.graphics.circb(c.x, c.y, c.r, c.c);
        c.r += 1;
      } else {
    ...
    this.pickups.forEach((p, i) => {
      p.y += 1;
      this.graphics.spr(p.t, p.x, p.y);
      if (p.y > this.graphics.ggh()) {
        this.pickups.splice(i, 1);
      }

      if (this.graphics.rrc(p, this.ship)) {
        this.pickups.splice(i, 1);
        switch (p.t) {
          case 32: {
            this.ship.l += 1;
    ...
    if (this.laserLine) {
      ...
      this.laserLine.y = 0;
      this.laserLine.h =
        this.graphics.ggh() - (this.graphics.ggh() - this.ship.y - 1);
  ...
  drawUI(): void {
    ...
    for (let i = 1; i < 4; i++) {
      if (this.ship.health < i) {
        this.graphics.spr(4, 200 + i * 10, 10);
      } else {
        this.graphics.spr(3, 200 + i * 10, 10);
      }
    }

    // draw player pickup status
    this.graphics.spr(32, 10, this.graphics.ggh() - 30);
    this.graphics.print("x" + this.ship.d, 16, this.graphics.ggh() - 33, 10, 2);
    this.graphics.spr(33, 10, this.graphics.ggh() - 20);
    this.graphics.print("x" + this.ship.l, 16, this.graphics.ggh() - 23, 10, 2);
    this.graphics.spr(35, 10, this.graphics.ggh() - 10);
    this.graphics.print("x" + this.ship.s, 16, this.graphics.ggh() - 13, 10, 2);

    // draw player score
    this.graphics.print("Score: " + this.ship.p, 5, 5, 10, 2);
  }

  addBackground(): void {
    for (let i = 0; i < 10; i++) {
      let s = {
        x: Math.floor(Math.random() * this.graphics.ggw()) + 1,
        y: Math.floor(Math.random() * this.graphics.ggw()) + 1,
        s: 2,
        c: 10,
      };
      this.stars.push(s);
  ...
  laserAttack(): void {
    this.laserLine = {
      ...
      y: this.ship.y - 1,
      w: 2,
      h: this.graphics.ggh() - this.ship.y,
      t: 2,
      c: 3,
      d: 100,
    };
